...
cd ${TRAVIS_BUILD_DIR}
rm -rf build
meson -DARCH=${ARCH} -DNDEBUG=true -DUNITTESTS=${UNITTESTS} -DPRINTING=${PRINTING} -DPROFILING=${PROFILING} -DCTRLC=${CTRLC} -DDFLOAT=${DFLOAT} -DDLONG=${DLONG} -DCOVERAGE=${COVERAGE} -DUSE_EMBEDDED=${USE_EMBEDDED} -DEMBEDDED=${EMBEDDED} build
ninja -C build -j8
