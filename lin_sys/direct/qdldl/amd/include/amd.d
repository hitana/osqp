...
enum c_int AMD_INFO = 20;          /* size of Info array */

/* contents of Control */
enum c_int AMD_DENSE = 0;       /* "dense" if degree > Control [0] * sqrt (n) */
enum c_int AMD_AGGRESSIVE = 1;  /* do aggressive absorption if Control [1] != 0 */

/* default Control settings */
enum c_float AMD_DEFAULT_DENSE = 10.0 ;   /* default "dense" degree 10*sqrt(n) */     
enum c_int AMD_DEFAULT_AGGRESSIVE = 1;        /* do aggressive absorption by default */

/* contents of Info */
enum c_int AMD_STATUS = 0;           /* return value of amd_order and amd_l_order */
enum c_int AMD_N = 1;               /* A is n-by-n */
enum c_int AMD_NZ = 2;     /* number of nonzeros in A */
enum c_int AMD_SYMMETRY = 3;        /* symmetry of pattern (1 is sym., 0 is unsym.) */
enum c_int AMD_NZDIAG = 4;           /* # of entries on diagonal */
enum c_int AMD_NZ_A_PLUS_AT = 5;  /* nz in A+A' */
enum c_int AMD_NDENSE = 6;           /* number of "dense" rows/columns in A */
enum c_int AMD_MEMORY = 7;           /* amount of memory used by AMD */
enum c_int AMD_NCMPA = 8;            /* number of garbage collections in AMD */
enum c_int AMD_LNZ = 9;     /* approx. nz in L, excluding the diagonal */
enum c_int AMD_NDIV = 10;            /* number of fl. point divides for LU and LDL' */
enum c_int AMD_NMULTSUBS_LDL = 11; /* number of fl. point (*,-) pairs for LDL' */
enum c_int AMD_NMULTSUBS_LU = 12;  /* number of fl. point (*,-) pairs for LU */
enum c_int AMD_DMAX = 13;            /* max nz. in any column of L, incl. diagonal */

/* ------------------------------------------------------------------------- */
/* return values of AMD */
/* ------------------------------------------------------------------------- */

enum c_int AMD_OK = 0;           /* success */
enum c_int AMD_OUT_OF_MEMORY = -1;        /* malloc failed, or problem too large */
enum c_int AMD_INVALID = -2;              /* input arguments are not valid */
enum c_int AMD_OK_BUT_JUMBLED = 1;        /* input matrix is OK for amd_order, but
...
enum string AMD_DATE = "May 4, 2016";
//#define AMD_VERSION_CODE(main,sub) ((main) * 1000 + (sub))
c_int AMD_VERSION_CODE(c_int main, c_int sub) { return (main) * 1000 + (sub); }
enum c_int AMD_MAIN_VERSION = 2;
enum c_int AMD_SUB_VERSION = 4;
enum c_int AMD_SUBSUB_VERSION = 6;
