...
void update_status(OSQPInfo *info, c_int status_val) {
  ...
  if (status_val == OSQP_SOLVED) c_strcpy(info.status.ptr, "solved");

  if (status_val == OSQP_SOLVED_INACCURATE) c_strcpy(info.status.ptr, "solved inaccurate");
  else if (status_val == OSQP_PRIMAL_INFEASIBLE) c_strcpy(info.status.ptr, "primal infeasible");
  else if (status_val == OSQP_PRIMAL_INFEASIBLE_INACCURATE) c_strcpy(info.status.ptr, "primal infeasible inaccurate");
  else if (status_val == OSQP_UNSOLVED) c_strcpy(info.status.ptr, "unsolved");
  else if (status_val == OSQP_DUAL_INFEASIBLE) c_strcpy(info.status.ptr,"dual infeasible");
  else if (status_val == OSQP_DUAL_INFEASIBLE_INACCURATE) c_strcpy(info.status.ptr, "dual infeasible inaccurate");
  else if (status_val == OSQP_MAX_ITER_REACHED) c_strcpy(info.status.ptr, "maximum iterations reached");
  else if (status_val == OSQP_TIME_LIMIT_REACHED) c_strcpy(info.status.ptr, "run time limit reached");
  else if (status_val == OSQP_SIGINT) c_strcpy(info.status.ptr, "interrupted");
  else if (status_val == OSQP_NON_CVX) c_strcpy(info.status.ptr, "problem non convex");
}
}
else {
  void update_status(OSQPInfo *info, c_int status_val) {
// Update status value
  ...
  if (status_val == OSQP_SOLVED_INACCURATE) c_strcpy(info.status.ptr,
                                                     "solved inaccurate");
  else if (status_val == OSQP_PRIMAL_INFEASIBLE) c_strcpy(info.status.ptr,
                                                          "primal infeasible");
  else if (status_val == OSQP_PRIMAL_INFEASIBLE_INACCURATE) c_strcpy(info.status.ptr,
                                                                     "primal infeasible inaccurate");
  else if (status_val == OSQP_UNSOLVED) c_strcpy(info.status.ptr, "unsolved");
  else if (status_val == OSQP_DUAL_INFEASIBLE) c_strcpy(info.status.ptr,
                                                        "dual infeasible");
  else if (status_val == OSQP_DUAL_INFEASIBLE_INACCURATE) c_strcpy(info.status.ptr,
                                                                   "dual infeasible inaccurate");
  else if (status_val == OSQP_MAX_ITER_REACHED) c_strcpy(info.status.ptr,
...
c_int validate_data(const OSQPData *data) {
  ...
  if (!data) {
version(PRINTING){
    c_eprint("ERROR in %s: Missing data\n", __FUNCTION__.ptr);
}
    return 1;
  }
  ...
  if (!(data.P)) {
version(PRINTING){
    c_eprint("ERROR in %s: Missing matrix P\n", __FUNCTION__.ptr);
}
    return 1;
  }
  ...
  if (!(data.A)) {
version(PRINTING){
    c_eprint("ERROR in %s: Missing matrix A\n", __FUNCTION__.ptr);
}
    return 1;
  }
...
version(PRINTING){
    c_eprint("ERROR in %s: n must be positive and m nonnegative; n = %i, m = %i\n", __FUNCTION__.ptr,
  ...
  if (data.P.m != data.n) {
version(PRINTING){
    c_eprint("ERROR in %s: P does not have dimension n x n with n = %i\n", __FUNCTION__.ptr, cast(int)data.n);
} /* ifdef PRINTING */
    return 1;
  }
...
version(PRINTING){
    c_eprint("ERROR in %s: P is not square\n", __FUNCTION__.ptr);
} /* ifdef PRINTING */
    return 1;
  }
...
version(PRINTING){
        c_eprint("ERROR in %s: P is not upper triangular\n", __FUNCTION__.ptr);
} /* ifdef PRINTING */
        return 1;
      }
    }
...
version(PRINTING){
    c_eprint("ERROR in %s: A does not have dimension %i x %i\n", __FUNCTION__.ptr, cast(int)data.m, cast(int)data.n);
} /* ifdef PRINTING */
    return 1;
  }
...
version(PRINTING){
      c_eprint("ERROR in %s: Lower bound at index %d is greater than upper bound: %.4e > %.4e\n", __FUNCTION__.ptr,
...
c_int validate_settings(const OSQPSettings *settings) {
  if (!settings) {
version(PRINTING){
    c_eprint("ERROR in %s: Missing settings!\n", __FUNCTION__.ptr);
} /* ifdef PRINTING */
    return 1;
  }
...
version(PRINTING){
    c_eprint("ERROR in %s: scaling must be nonnegative\n", __FUNCTION__.ptr);
} /* ifdef PRINTING */
    return 1;
  }
...
version(PRINTING){
    c_eprint("ERROR in %s: adaptive_rho must be either 0 or 1\n", __FUNCTION__.ptr);
} /* ifdef PRINTING */
    return 1;
  }
...
version(PRINTING){
    c_eprint("ERROR in %s: adaptive_rho_interval must be nonnegative\n", __FUNCTION__.ptr);
} /* ifdef PRINTING */
    return 1;
  }
version(PROFILING){
...
version(PRINTING){
    c_eprint("ERROR in %s: adaptive_rho_fraction must be positive\n", __FUNCTION__.ptr);
} /* ifdef PRINTING */
    return 1;
  }
} /* ifdef PROFILING */
...
version(PRINTING){
    c_eprint("ERROR in %s: adaptive_rho_tolerance must be >= 1\n", __FUNCTION__.ptr);
} /* ifdef PRINTING */
    return 1;
  }
...
version(PRINTING){
    c_eprint("ERROR in %s: sigma must be positive\n", __FUNCTION__.ptr);
} /* ifdef PRINTING */
    return 1;
  }
...
version(PRINTING){
    c_eprint("ERROR in %s: at least one of eps_abs and eps_rel must be positive\n", __FUNCTION__.ptr);
} /* ifdef PRINTING */
    return 1;
  }
...
version(PRINTING){
    c_eprint("ERROR in %s: eps_prim_inf must be positive\n", __FUNCTION__.ptr);
} /* ifdef PRINTING */
    return 1;
  }
...
version(PRINTING){
    c_eprint("ERROR in %s: eps_dual_inf must be positive\n", __FUNCTION__.ptr);
} /* ifdef PRINTING */
    return 1;
  }
  ...
  if ((settings.alpha <= 0.0) ||
      (settings.alpha >= 2.0)) {
version(PRINTING){
    c_eprint("ERROR in %s: alpha must be strictly between 0 and 2\n", __FUNCTION__.ptr);
} /* ifdef PRINTING */
    return 1;
  }
...
version(PRINTING){
    c_eprint("ERROR in %s: linsys_solver not recognized\n", __FUNCTION__.ptr);
} /* ifdef PRINTING */
    return 1;
  }
  ...
  if ((settings.verbose != 0) &&
      (settings.verbose != 1)) {
version(PRINTING){
    c_eprint("ERROR in %s: verbose must be either 0 or 1\n", __FUNCTION__.ptr);
} /* ifdef PRINTING */
    return 1;
  }
...
version(PRINTING){
    c_eprint("ERROR in %s: scaled_termination must be either 0 or 1\n", __FUNCTION__.ptr);
} /* ifdef PRINTING */
    return 1;
  }
...
version(PRINTING){
    c_eprint("ERROR in %s: check_termination must be nonnegative\n", __FUNCTION__.ptr);
} /* ifdef PRINTING */
    return 1;
  }
...
version(PRINTING){
    c_eprint("ERROR in %s: warm_start must be either 0 or 1\n", __FUNCTION__.ptr);
} /* ifdef PRINTING */
    return 1;
  }
version(PROFILING){
...
version(PRINTING){
    c_eprint("ERROR in %s: time_limit must be nonnegative\n", __FUNCTION__.ptr);
} /* ifdef PRINTING */
    return 1;
  }
} /* ifdef PROFILING */
