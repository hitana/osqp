...
enum string OSQP_VERSION = "0.6.0";
...
enum int OSQP_DUAL_INFEASIBLE_INACCURATE = 4;
enum int OSQP_PRIMAL_INFEASIBLE_INACCURATE = 3;
enum int OSQP_SOLVED_INACCURATE = 2;
enum int OSQP_SOLVED = 1;
enum int OSQP_MAX_ITER_REACHED = -2;
enum int OSQP_PRIMAL_INFEASIBLE = -3;    /* primal infeasible  */
enum int OSQP_DUAL_INFEASIBLE = -4;      /* dual infeasible */
enum int OSQP_SIGINT = -5;               /* interrupted by user */
version (PROFILING)
{
    enum int OSQP_TIME_LIMIT_REACHED = -6;
}
enum int OSQP_NON_CVX = -7;              /* problem non convex */
enum int OSQP_UNSOLVED = -10;            /* Unsolved. Only setup function has been called */
...
// todo : for qdldl_interface compilaton
...
enum float RHO = 0.1;
enum float SIGMA = 1E-06;
enum int MAX_ITER = 4000;
enum float EPS_ABS = 1E-3;
...
enum float RHO_MAX = 1e06;
enum float RHO_EQ_OVER_RHO_INEQ = 1e03;
enum float RHO_TOL = 1e-04; ///< tolerance for detecting if an inequality is set to equality
...
//#  define VERBOSE (1)
    enum float DELTA = 1E-6;
    enum int POLISH = 0;
    enum int POLISH_REFINE_ITER = 3;
    enum int VERBOSE = 1;
} // ifndef EMBEDDED
...
enum int SCALED_TERMINATION = 0;
enum int CHECK_TERMINATION = 25;
enum int WARM_START = 1;
enum int SCALING = 10;
enum c_float MIN_SCALING = 1e-04;   ///< minimum scaling value
enum c_float MAX_SCALING = 1e+04;   ///< maximum scaling value
...
enum c_float OSQP_NAN = (cast(c_float)0x7fc00000UL);  // not a number

//# ifndef OSQP_INFTY
//#  define OSQP_INFTY ((c_float)1e30)        // infinity
//# endif /* ifndef OSQP_INFTY */
enum c_float OSQP_INFTY = (cast(c_float)1e30) ; // infinity
...
//#  define ADAPTIVE_RHO_TOLERANCE (5)            ///< tolerance for adopting new rho; minimum ratio between new rho and the current one
    enum int ADAPTIVE_RHO = 1;
    enum int ADAPTIVE_RHO_INTERVAL = 0;
    enum c_float ADAPTIVE_RHO_FRACTION = 0.4;        ///< fraction of setup time after which we update rho
    enum int ADAPTIVE_RHO_MULTIPLE_TERMINATION = 4;  ///< multiple of check_termination after which we update rho (if PROFILING disabled)
    enum int ADAPTIVE_RHO_FIXED = 100;               ///< number of iterations after which we update rho if termination_check  and PROFILING are disabled
    enum int ADAPTIVE_RHO_TOLERANCE = 5;             ///< tolerance for adopting new rho; minimum ratio between new rho and the current one

} // if EMBEDDED != 1

version (PROFILING) {
    //#  define TIME_LIMIT (0)                        ///< Disable time limit as default
    enum int TIME_LIMIT = 0;                          ///< Disable time limit as default
} // ifdef PROFILING

/* Printing */
//# define PRINT_INTERVAL 200
enum int PRINT_INTERVAL = 200;
